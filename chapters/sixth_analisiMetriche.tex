...
di circa 6–8 ms, con oscillazioni sufficentemente moderante con delle bande di
dispersione relativamente strette. Il delay medio è circa 7.00 ms, con una
mediana 6.82 ms e un intervallo interquartile uguale ~1.95 ms. La media della
banda interquartile di circa 1.43 ms, invece la larghezza media della banda p10–p90
è di circa 2.85 ms.

Nel caso \texttt{medium}, il delay cresce in modo chiaro e si posiziona su una fascia
superiore rispetto al caso precedente. La curva media si posiziona
prevalentemente tra circa 16 e 19 ms per una parte consistente dell’osservazione,
per poi diminuire in fase finale verso valori ~10 ms. Il delay medio è ~15.5 ms, con
mediana 16.5 ms e un intervallo interquartile di ~5.18 ms. L'IQR lungo la curva è
~5.62 ms e la p10–p90 ~10.0 ms. Complessiavmente l'andamento rimane leggibile e
...
regolare. Dopo la peer-selection, la curva media si posiziona in fase iniziale
su valori molto elevati, spesso compresi tra ~(40-65) ms, con oscillazioni ampie e
una forte eterogeneità tra run, evidenziato dall'ampliamento delle bande di
dispersione. Il ivello medio è verso valori ~(10–15) ms. Il delay medio è pari a ~35.0
ms, con mediana 46.7 ms e l'intervallo interquartile molto ampio, pari a ~59.9 ms.
Le statistiche della curva aggregata vanno confermano il trend, infatti si ha una
...
mantenendo una dinamica ancora sufficientemente controllata. Lo scenario \texttt{high}
mostra invece una discontinuità più marcata. Si ha un livello medio spostato su valori
più alti e lo stesso comportamento vale per la variabilità. In termini
qualitativi, la metrica del delay risulta quindi efficace nel differenziare i tre
scenari e nel mostrare come l’incremento del degrado di rete si traduca, per il
nodo client NTPsec, in un peggioramento sia del livello della misura sia della sua
...
L’analisi del jitter misurato sul nodo client per il demone NTPsec evidenzia un
cambiamento nei tre scenari. La metrica mostra un incremento progressivo dell'livello
medio e della variabilità.

Nello scenario \texttt{low}, il jitter rimane su valori contenuti e relativamente
regolari. Il valore medio risulta ~1.05 ms, con una dispersione moderata. Si hanno:
banda interquartile media della curva aggregata è pari a ~0.65 ms, invece la
banda p10--p90 si attesta intorno a ~1.46 ms. Il comportamento osservato è coerente
con un degrado debole, evidenziando una condizione di rete stabile, nella quale il
client mantiene una sincronizzazione sufficientemente regolare.

Nel caso \texttt{medium}, il jitter cresce in modo evidente e si posiziona su una
fascia sensibilmente superiore.Il valore medio è ~3.14 ms, all'incirca tre volte il
corrispondente valore osservato nello scenario \texttt{low}. Contestualmente si ha
un aumento della dispersione, con banda interquartile media di ~2.40 ms e banda p10--p90
di ~4.18 ms. Il sistema resta stabile, nonostante la sincronizzazione risulta più
sensibile agli impairment di rete.

Nello scenario \texttt{high} si manifesta un degradato più marcato. Il jitter assume
valori elevati lungo l’intera serie aggregata, con una media all'incirca di 20.9
ms. Sia ha un incremento rispetto agli scenari precedenti, in particolare
rispetto allo scenario \texttt{low}, con un divario di un ordine di grandezza. La
dispersione misruata aumenta con banda interquartile media ~9.38 ms e banda p10--p90
di circa 18.3 ms. Nel grafico si evidenzia una forte oscillazione, ampia e persiste.

In generale, vie è aumento coerente con il degrado di rete, evidenziando un impatto
...
Nell’analisi dell’offset si evidenziano valori sistematicamente negativi dopo la
selezione del peer, indicando una deviazione del clock locale rispetto al riferimento
temporale.

Nello scenario \texttt{low}, l’offset si stabilizza su valori contenuti e regolari,
posizionandosi in valori ~$-2.5$/$-3\,\mathrm{ms}$. La media è circa di
$-2.55\,\mathrm{ms}$ con una mediana pressoché identica di $-2 .55\,\mathrm{ms}$.
La dispersione è moderata, ($\sigma \approx 0.91\,\mathrm{ms}$). La curva aggregata
conferma il trend con un valore medio centrale di $-2.55\,\mathrm{ms}$, invece
l’ampiezza banda interquartile è di ~($\mathrm{IQR}\approx 0.56\,\mathrm{ms}$) e la
fascia $p10$--$p 90$ di circa ($\approx 1.21\,\mathrm{ms}$).Il comportamento è
stabile e uno scostamento limitato rispetto al riferimento.

Nel caso \texttt{medium} si ha un offset traslato su valori più marcati (negativi).
La misura posiziona in una fascia prossima a $-7$/$-8\,\mathrm{ms}$, con un aumento
nella fase finale della serie. La media è ~$-6.70\,\mathrm{ms}$, con mediana
pari a $-7.08\,\mathrm{ms}$ e deviazione standard di circa $2.81\,\mathrm{ms}$. La
curva aggregata rimane coerente con un valor medio centrale di $-6.69\,\mathrm{ms}$.
Vi è una crescita anche della variabilità, mostrato dall’ampiezza media della banda
interquartile ($\approx 2.74\,\mathrm{ms}$) e della fascia $p10$--$p90$ ($\approx
4.97\,\mathrm{ms}$). Vi è un incremento dell'offset rispetto all'scenario precedente.

Lo scenario \texttt{high}, conferma il trend. L’offset è caratterizzato da
volori negativamente elevati, con ampie escursioni. La curva aggregata evidenzia
una prolungata permanenza dei valori in una fascia di circa $-20$ e
...
confermano quanto osservato. Gli indicatori dalla curva aggregata evidenziano una
dispersione superiore rispetto agli altri scenari. L’ampiezza media della banda
interquartile è di ~$15 .17\,\mathrm{ms}$, La fascia $p10$--$p90$ di ~$29.44\, \mathrm{ms}$.
Il minimo della curva media aggregata scende inoltre fino a circa $-30.12\,\mathrm{ms}$.

La misura dell'offset conferma un coerente e progressivo degrado al variare degli
impairment. Da \texttt{low} a \texttt{medium} si osserva un incremento netto del
modulo dell’offset e della dispersione associata invece da \texttt{medium} a \texttt{high}
il peggioramento risulta più marcato, caratterizzato da statistiche che ne confermano
l'andamento.
...
ms in low, 0.442 ms in medium e 0.473 ms in high. Diffrenze ridotte, evidenziando
una assenza di degrado del nodo \texttt{boundary}.La variabilità delle curve
...
I risultati ottenuti mostrano quindi un assenza di degrado, diffrentemente con
quanto osservato sul nodo \texttt{clientntp}. Le differenze osservate tra low, medium
e high risultano contenute e prive di una tendenza sistematica. I dati ottenuti
...
Per quanto rigurada, l’analisi quantitativa delle metriche, non può essere estesa
...
centrali piuttosto contenuta, con un intervallo interquartile nell'intervallo
...
L’intervallo interquartile della curva centrale è nell'intervallo di
$613.0\,\mu s$ e $672.3\, \mu s$, invece le bande di variabilità sono mediamente più
ampie. L’ampiezza dell’IQR sia quella della fascia $p10$--$p 90$, segnalando una
minore regolarità della stima del ritardo di percorso.

Vi è un chiaro aumento del degrado tra i primi due scenari, in termini misurazioni
otteute. Nonostante il degrado della rete non produce un forte aumento del
...
L’intervallo interquartile dei valori centrali è maggiore e le bande di
...
Differenze contennute nei tre casi, non indicano quindi una variazione sostanziale.
L'ampiezza interquartile media e la banda p10--p90 non mostrano infatti un incremento
...
banda interquartile media è contenuta, con un valore di ~\texttt{1.095 \,$\mu$s}.
...
\,$\mu$s}. La banda interquartile media cresce a \texttt{1.804 \,$\mu$s}; inoltre,
...
Nel caso \texttt{medium}, c'è un comportamente più regolare. Si ha una media aggregata
di ~\texttt{-3.863 \,$\mu$s}. La dispersione è ridotta notevolmentela deviazione
...
medie che si spostano verso valori più negativi e con un aumento della dispersioneNel
...
\end{figure}
\paragraph*{clientchrony - netem/tc:}
Per la metrica \texttt{stddev\_us}, il caso \texttt{clientchrony} è presente una variazione
...
texttt{low} occupa una posizione intermedia e \texttt{high} mostra invece valori
...
standard delle medie è di \texttt{9.885 \,$\mu$s}. I grafici evideniano oscillazioni
