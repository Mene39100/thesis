...
\begin{lstlisting}[language=bash,caption={Schema generale di applicazione di netem e tbf.},label={lst:tc-netem-tbf-generale}]
...
\begin{lstlisting}[language=bash,caption={Rimozione manuale di una qdisc da un'interfaccia.},label={lst:tc-cleanup}]
...
\begin{lstlisting}[language=bash,caption={Esempio di file di scenario low.conf.},label={lst:low-conf}]
DELAY="5ms"
JITTER="1ms"
LOSS="0.1%"
REORDER="1%"
...
\begin{lstlisting}[language=bash,caption={Selezione dell'interfaccia degradata per PTP e NTPsec.},label={lst:ptp-ntpsec-iface}]
NETNS="boundary"
IFACE="eth1"   # boundary lato rete B
\end{lstlisting}
...
\begin{lstlisting}[language=bash,caption={Applicazione di netem e tbf su boundary:eth1.},label={lst:tc-boundary-eth1}]
...
\begin{lstlisting}[language=bash,caption={Applicazione di netem e tbf su clientchrony:eth0.},label={lst:tc-clientchrony}]
CLIENT_IFACE="eth0"
...
\begin{lstlisting}[language=bash,caption={Applicazione di netem e tbf su servergm:eth0.},label={lst:tc-servergm}]
CLIENT_IFACE="eth0"
...
\begin{lstlisting}[language=bash,caption={Variabili globali dello script di automazione.},label={lst:auto-vars}]
SCENARIOS=("low" "medium" "high")
SCENARIO_DIR="$(dirname "$0")/scenarios"
RAWLOG_ROOT="analysis/raw_logs/T3_multiplerun"
NETNS="boundary"
...
\begin{lstlisting}[language=bash,caption={Funzioni principali dello script di automazione.},label={lst:auto-functions}]
run_ptp_scenario() {
    ...
}
...
\begin{lstlisting}[language=bash,caption={Creazione della directory di output per una run PTP.},label={lst:auto-run-dir-ptp}]
local S="$1"
local RUN_ID="$2"
...
\begin{lstlisting}[language=bash,caption={Importazione dei parametri dello scenario corrente.},label={lst:auto-source-scenario}]
...
\begin{lstlisting}[language=bash,caption={Pulizia e avvio della topologia prima di una run.},label={lst:auto-lstart}]
kathara lclean -d "$TOPOLOGY_DIR" || true
...
\begin{lstlisting}[language=bash,caption={Avvio dei processi PTP nella run automatizzata.},label={lst:auto-ptp-start}]
kathara exec -d "$TOPOLOGY_DIR" servergm -- bash -lc \
  "pkill chronyd 2>/dev/null || true; \
  sleep 1; \
  ptp4l -f /etc/ptp/server_gm.conf -m -i eth0 -S \
...
\begin{lstlisting}[language=bash,caption={Fase di stabilizzazione PTP e chiusura della run.},label={lst:auto-ptp-sleep}]
...
\begin{lstlisting}[language=bash,caption={Avvio controllato di NTPsec sul boundary.},label={lst:auto-ntpsec-start}]
kathara exec -d "$TOPOLOGY_DIR" boundary -- bash -lc \
...
\begin{lstlisting}[language=bash,caption={Loop di logging NTPsec sul client.},label={lst:auto-ntpsec-client-loop}]
...
\begin{lstlisting}[language=bash,caption={Durata della fase di raccolta NTPsec.},label={lst:auto-ntpsec-sleep}]
sleep 1500
...
\begin{lstlisting}[language=bash,caption={Directory di output per le due modalità Chrony.},label={lst:auto-chrony-dirs}]
...
\begin{lstlisting}[language=bash,caption={Salvataggio dello stato della qdisc in una run Chrony.},label={lst:auto-chrony-qdisc-state}]
kathara exec -d "$TOPOLOGY_DIR" clientchrony -- bash -lc \
...
\begin{lstlisting}[language=bash,caption={Inizializzazione dei file di output Chrony.},label={lst:auto-chrony-files}]
SAMPLES=20
INTERVAL=60
...
\begin{lstlisting}[language=bash,caption={Raccolta periodica delle metriche Chrony.},label={lst:auto-chrony-loop}]
...
\begin{lstlisting}[language=bash,caption={Ciclo finale di esecuzione delle run.},label={lst:auto-final-loop}]
